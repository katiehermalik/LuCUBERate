      ...
      if (currentCategory) {
        if ((currentPath[0] === 'new' || 
            (currentPath[0] === 'edit' && currentCubeCategory !== currentCategory))) {
              ...
              setCurrentCategoryData(categories.find(category => category._id === currentCategory))
        } else {
          setRenderPlaceHolder(false);
        }
      }
    },[currentCategory,categories,currentCubeId,currentPath,pathname,currentCubeCategory])
