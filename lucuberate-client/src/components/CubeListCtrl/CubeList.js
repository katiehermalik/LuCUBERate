import React, { useContext, useReducer, useRef, useState, useCallback, useEffect } from 'react';
...
const CubeList = ({ history, history:{location:{pathname}}}) => {
  ...
  const { currentUserInfo: { cubes } } = useContext(UserContext);
        ...
        <div
          ...
          ?
            <div className="container-row pointer-disabled">
              <CubeCount categoryCubeLength={cubeListLength} cubeListOpened={true} />
              <span 
              className="container-row pointer-auto">
                {currentCubeId && currentPath[0] === 'show' && currentCubeCategory === currentCategory && currCategoryCubeRefs.length > 1 &&
                  ...
                  <DeleteBtn categoryId={categoryId} categoryTitle={categoryTitle} />
              </span>
          ...
          : 
            <div className="container-row pointer-disabled">
              <CubeCount categoryCubeLength={cubeListLength} cubeListOpened={false} />
              <i className="icon-chevron pointer-disabled">
          ...
          {categoryCubes?.map((cube, j) => 
            <li key={cube} className="radio-button">
              <input
                ...
                ref={element => {
                  if (element) {
                    cubeRefs[i][j] = {category_id: categoryId, ref: element};
                  }
                }}/>
              <label
